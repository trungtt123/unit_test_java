...
public class GetListBrandsRestAssured extends BaseRestAssuredClass{

	private static String[] listUnitTest = Constant.API_GET_LIST_BRANDS;
    ...
    public static ResponseUnitTest UnitTest000() {
		...
		try {
			...
			Response res = given()
	                ...
	                .when()
	                .get(RequestApiGetListBrands.apiPath);

			
			rut.setName(listUnitTest[0]);
	        rut.setOutput(res.asPrettyString());
			
			ListBrandsResponseModel resObj = mapper.readValue(res.asString(), ListBrandsResponseModel.class);
